...
class JunctionTree:
    ...
    def _incorporate_evidence(self, jt: nx.DiGraph, evidence: Dict[str, int]) -> nx.DiGraph:
        ...
        # For each variable in evidence, find the node in the junction tree that has the same name as the variable
        # and reduce the factor table of that node to the set of value combinations (index) matching the value of the
        # observed evidence variable.
        for var, val in evidence.items():
            for node in result.nodes():
                if var in result.nodes[node]['factor_vars']:
                    result.nodes[node]['factor_table'] = result.nodes[node]['factor_table'].loc[val]

        return result

    def marginalize(self, jt: nx.DiGraph, query_vars: List[str]) -> Dict[str, float]:
        """
        Compute the marginal distribution of each variable in the bayesian network.
        :return: A dictionary of the marginal distributions of each variable in the bayesian network.
        """
        result = {}
        for var in query_vars:
            # Find the node in the junction tree that has the same name as the variable
            for node in jt.nodes():
                if var in jt.nodes[node]['factor_vars']:
                    # Compute the marginal distribution of the variable
                    result[var] = jt.nodes[node]['factor_table'].sum(axis=0)
                    break
        return result

    def _run_upward_pass(self, jt: nx.DiGraph) -> None:
        """
        Run the upward pass of belief propagation.
        :param jt: the initial uncalibrated Junction Tree with factors loaded from the original bayesian network
        :return: None
        """
        raise NotImplementedError("asjdlaksjdlkasj")
    ...
    def _run_belief_propagation(self, uncalibrated_jt) -> nx.DiGraph:
        ...
        # Run the upward pass
        calibrated_jt = self._run_upward_pass(uncalibrated_jt)
